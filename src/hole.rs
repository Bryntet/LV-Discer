...
impl<'results> RoundStats<'results> {
    fn new(round_number: u8, holes: [(u8, &'results [SimpleResult]); 18]) -> Self {
        Self {
            round: round_number,
            holes: holes.map(|(hole_number,results)|{
                HoleStats::new(hole_number,results)
            })
        }
    }
}
...
impl<'results> HoleStats<'results> {
    ...
    pub fn average_score(&self) -> i8 {
        (self.player_results.iter().map(|res| res.score).sum::<f64>() / (self.player_results.len() as f64)).round() as i8
    }
}