...
pub async fn set_focus(focused_player: usize, coordinator: Coordinator, updater: &State<GeneralChannel<GroupSelectionUpdate>>) -> Json<dto::Player> {
...
#[openapi(tag = "Config")]
#[post("/group/<group_id>")]
pub async fn set_group(coordinator: Coordinator, group_id: &str, updater: &State<GeneralChannel<GroupSelectionUpdate>>) -> Result<(), &'static str> {
    let mut coordinator = coordinator
        .lock()
        .await;
    coordinator
