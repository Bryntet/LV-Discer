...
impl TjingResultMap {
    pub fn new(players: Vec<&Player>) -> Self {
        ...
        Self {
            results: players.into_iter().cloned().map(|player|(player.player_id,player.results.tjing_results())).collect(),
        }
    }

    #[inline(always)]
    pub fn update(&mut self, player_id: &str, hole_result: crate::controller::coordinator::queries::HoleResult) -> bool {
    ...
    #[inline(always)]
    pub fn update_many(&mut self, player_id: &str, hole_results: Vec<crate::controller::coordinator::queries::HoleResult>) -> bool {
        let mut needs_update = false;
...
pub async fn update_loop(coordinator: Arc<Mutex<FlipUpVMixCoordinator>>) {
    ...
    let tjing_request = crate::controller::coordinator::queries::RoundResultsQuery::build(RoundResultsQueryVariables {
    ...
    loop {
        if let Ok(response) = reqwest_client
            ...
            .send()
            .await {
            if let Ok(GraphQlResponse {data: Some(RoundResultsQuery{event: Some(controller::queries::Event{players,..})}), ..} ) = response.json::<GraphQlResponse<RoundResultsQuery>>().await {
                if tjing_result_map.update_all_players(players.into_iter().flat_map(|player| Some((player.id.into_inner(),player.results?))).collect_vec()) {
