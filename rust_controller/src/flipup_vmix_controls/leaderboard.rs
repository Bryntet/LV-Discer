...
impl Leaderboard {
    ...
    fn current_state(&self) -> &LeaderboardState {
        self.states.last().expect("Leaderboard has to have state when initiated")
    }
    ...
    pub fn to_vmix_instructions(&self) -> Vec<VMixFunction<LeaderBoardProperty>> {
        self.current_state().to_vmix_instructions(self.previous_state())
    }
}
impl LeaderboardState {
    ...
    fn leaderboard_players(&self, other: Option<&Self>) -> Vec<LeaderboardPlayer> {
        let max_score = self.players.iter().map(|p|p.round_score).max().unwrap_or_default();
        ...
        players_with_pos.into_iter().enumerate().map(|(real_pos,(index,player))| {
    ...
    pub fn to_vmix_instructions(&self, other: Option<&Self>) -> Vec<VMixFunction<LeaderBoardProperty>> {
        let players = self.leaderboard_players(other);
        players.iter().map(LeaderboardPlayer::combine).flatten().collect_vec()
    }
}
...
impl LeaderboardPlayer {
    ...
    pub fn new(player: &Player, pos: usize,index:usize, max_score_reached:isize, other_board: Option<&Vec<&Player>>) -> Self {
        let other_pos = other_board.and_then(|players|players.iter().enumerate().find(|(_,other_player)|other_player.name== player.name)).map(|(pos,_)|pos);
        let movement = match other_pos {
            Some(other_pos) => {
                LeaderboardMovement::new(pos,other_pos)
            },
            None => {
                LeaderboardMovement::Same
            }
        };
        LeaderboardPlayer{
            position: pos,
    ...
    fn set_position(&self) -> VMixFunction<LeaderBoardProperty> {
        VMixFunction::SetText {
            ...
            input: LeaderBoardProperty::Position{pos: self.position, lb_pos: self.position, tied: false}.into()
        }
    }
...
mod prop {
    ...
    pub enum LeaderBoardProperty {
        Position { pos: usize, lb_pos: usize, tied: bool },
        Name(usize),
        HotRound(usize),
        RoundScore(usize),
