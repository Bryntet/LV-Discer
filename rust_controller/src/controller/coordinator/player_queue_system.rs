...
impl PlayerManager {
    ...
    fn focused_player(&self) -> &PlayerWithQueue {
        self.managed_players.get(self.focused).expect("We need to have a focused player")
    }
    ...
    fn next_queue_position(&self) -> usize {
        self.managed_players.iter().flat_map(|player|player.queue_position).max().unwrap_or(0) + 1
    }
    ...
    pub fn add_to_queue(&mut self,id: String) {
        ...
        if let Some(player) = self.managed_players.iter_mut().find(|player|player.player_id == id) {
    ...
    pub fn queued_players(&self) -> Vec<&PlayerWithQueue> {
        self.managed_players.iter().filter(|player|player.queue_position.is_some()).collect_vec()
    }
    ...
    pub fn players<'a>(&self, players: Vec<&'a Player>) -> Vec<&'a Player> {
        ...
        for player in &self.managed_players {
            if let Some(player) = players.iter().find(|other_player|player.player_id==other_player.player_id) {
